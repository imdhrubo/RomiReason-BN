...
#SBATCH --job-name=rrbn-llama8-v13
#SBATCH --output=slurm-rrbn-llama8-v13-%j.out
#SBATCH --error=slurm-rrbn-llama8-v13-%j.err
set -euo pipefail
...
python scripts/hpc/run_vllm_jsonl.py --jobs data/evaluation/primary_v1_3_answer_only_inference_inputs/llama-3-1-8b-instruct.jsonl --shard-size 500 --all-shards --output-dir "$RESULTS_ROOT/primary_v1_3_answer_only/llama-3-1-8b-instruct/responses" --event-dir "$RESULTS_ROOT/primary_v1_3_answer_only/llama-3-1-8b-instruct/events" --tensor-parallel-size 2 --dtype bfloat16
