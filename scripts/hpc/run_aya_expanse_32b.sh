...
#SBATCH --output=slurm-rrbn-aya32b-%A_%a.out
...
python scripts/hpc/run_vllm_jsonl.py \
  ...
  --shard-size 500 \
  --output "results/primary_v1_1/aya-expanse-32b/responses/shard-$(printf '%05d' "$SLURM_ARRAY_TASK_ID").jsonl" \
  --event-log "results/primary_v1_1/aya-expanse-32b/events/shard-$(printf '%05d' "$SLURM_ARRAY_TASK_ID").jsonl" \
